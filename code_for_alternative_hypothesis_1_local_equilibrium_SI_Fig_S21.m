...
% alternative architecture 1: local equilibrium between cells and air 
% spaces. We generate 3 predictions (mean and bounds of uncertainty 
...
bs_tortuosity_arr=[1, 2, 3]; % bundle sheath tortuosity (dimensionless)
mes_tortuosity_arr=[1, 2, 3]; % mesophyll tortuosity (dimensionless)
k_cc_max_arr=[12e-6, 11e-6, 10e-6]; % maximum cell-to-cell conductivity in mol/m/s/MPa
k_cc_min_arr=[7.5e-8, 7e-8, 6.5e-8]; % minimum cell-to-cell conductivity in mol/m/s/MPa
k_cc_trigger_arr=[-0.24e6, -0.23e6, -0.22e6]; % xylem stress at which 'characteristic' loss of cell-to-cell conductivity occurs (in Pa)
k_cc_decline_sensitivity_arr=[0.9e-5, 1e-5, 1.1e-5]; % determines sensitivity of cell-to-cell conductivity to xylem stress in MPa^(-1)
...
kappa_mes_mem=7e-6; % hydraulic conductivity of mesophyll membranes at cell-air space interface in mol/m/s/MPa: Rockwell review undersaturation - use higher end of L_p 
kappa_bs_mem=kappa_mes_mem; % hydraulic conductivity of bundle sheath membranes at cell-air space interface in mol/m/s/MPa: Rockwell review undersaturation - use higher end of L_p 
...
for ii=1:3
    ...
    k_cc_max=k_cc_max_arr(ii); 
    k_cc_decline_sensitivity=k_cc_decline_sensitivity_arr(ii); 
    k_cc_trigger=k_cc_trigger_arr(ii); 
    k_cc_min=k_cc_min_arr(ii); 
    % sigmoid function
    k_cc_arr=k_cc_max*1./(1+exp(-k_cc_decline_sensitivity*(psi_xylem-k_cc_trigger)))+k_cc_min; 
    
    % plot kappa_mem vs psi_xyl
    figure(1);hold on;
    plot(psi_xylem/1e6,k_cc_arr,'k-','linewidth',2);hold on;
    pbaspect([1 1 1]);
    ylabel('k_{c-c} (mol/m/s/MPa)');ylim([1e-8 2e-5]);
    ...
    for i=1:length(psi_xylem)
        ...
        k_cc=k_cc_arr(i); % Cell-to-cell conductivity 
        ...
        k_cc_exp=floor(log10(k_cc));
        k_cc_a=k_cc/(10^k_cc_exp);
        ...
        title(sprintf('k_{c-c} = %.2f \\times 10^{%d} mol/m/s/MPa',k_cc_a,k_cc_exp),'FontSize',20,'Interpreter','tex');
