...
% generate 3 predictions (mean and bounds of uncertainty interval) with 
...
kappa_mem_max_arr=[12e-6, 11e-6, 10e-6]; % maximum membrane conductivity in mol/m/s/MPa
kappa_mem_min_arr=[4.5e-8, 4.1e-8, 3.7e-8]; % minimum membrane conductivity in mol/m/s/MPa
kappa_mem_trigger_arr=[-0.24e6, -0.22e6, -0.2e6]; % xylem stress at which 'characteristic' loss of membrane conductivity occurs (in Pa)
kappa_mem_decline_sensitivity_arr=[0.9e-5, 1.1e-5, 1.3e-5]; % determines sensitivity of membrane conductivity to xylem stress in MPa^(-1)

% Fig. 4f (well-watered) corresponds to psi_xyl = -0.4 +/- 0.15 MPa: use VPD=3.8 kPa; i=7th element in psi_xyl array for ii=1,2,3 to generate Fig. 4f
% Fig. 4g (water-limited) corresponds to psi_xyl = -0.85 +/- 0.15 MPa: use VPD=4.2 kPa; i=14 (for ii=3), i=16 (for ii=2), and i=18 (for ii=1) to generate Fig. 4g
% Fig. 4h (abaxial-taped) corresponds to psi_xyl = -1.2 +/- 0.15 MPa: use VPD=4.2 kPa; gs_ad=75e-3, gs_ab=0; ii=1: i=17; ii=2: i=20; ii=3: i=24 to generate Fig. 4h
...
k_cc=5e-5; % hydraulic conductivity of cell-to-cell path in mol/m/s/MPa: Rockwell review undersaturation - use higher end of L_p 
...
for ii=1:3
    ...
    kappa_mem_max=kappa_mem_max_arr(ii); 
    kappa_mem_decline_sensitivity=kappa_mem_decline_sensitivity_arr(ii); 
    kappa_mem_trigger=kappa_mem_trigger_arr(ii); 
    kappa_mem_min=kappa_mem_min_arr(ii); 
    % sigmoid function
    kappa_mem_arr=kappa_mem_max*1./(1+exp(-kappa_mem_decline_sensitivity*(psi_xylem-kappa_mem_trigger)))+kappa_mem_min; 
    
    % plot kappa_mem vs psi_xyl
    figure(1);hold on;
    plot(psi_xylem/1e6,kappa_mem_arr,'k-','linewidth',2);hold on;
    pbaspect([1 1 1]);
    ylabel('\kappa_{mem} (mol/m/s/MPa)');ylim([1e-8 2e-5]);
    ...
    for i=1:length(psi_xylem)
        ...
        kappa_mes_mem=kappa_mem_arr(i); % mesophyll membrane conductivity 
        kappa_bs_mem=kappa_mes_mem; % bundle sheath membrane conductivity 
        ...
        for j=1:length(rel_hum)
            ...
            % r3 and s3 are the roots of the auxiliary equation: see SI equation S43
            r3=+sqrt(a1+a2);
            s3=+sqrt(b1+b2);
            ...
            x=fsolve(@(x)solver_separate_bs_and_mesophyll(x,psi_xylem(i),rel_hum(j),a1,a2,b1,b2,r3,s3,L_ad,L_ab,L_bs,gs_ad,gs_ab,p_sat,v_bar,R,leaf_temp,k_mes_vap,k_bs_vap,k_mes_cw,k_bs_cw,p_atm,A_cc,A_vap,A_cw),init);
            
            %  psi_bs
            psi_bs_cc_ad_arr=x(1) + x(2)*coords_bs_ad + x(3)*exp(r3*coords_bs_ad) + x(4)*exp(-r3*coords_bs_ad);
            psi_bs_apo_ad_arr=x(1) + x(2)*coords_bs_ad - a2*x(3)*exp(r3*coords_bs_ad)/a1 - a2*x(4)*exp(-r3*coords_bs_ad)/a1;
            psi_bs_cc_ab_arr=x(5) + x(6)*coords_bs_ab + x(7)*exp(r3*coords_bs_ab) + x(8)*exp(-r3*coords_bs_ab);
            psi_bs_apo_ab_arr=x(5) + x(6)*coords_bs_ab - a2*x(7)*exp(r3*coords_bs_ab)/a1 - a2*x(8)*exp(-r3*coords_bs_ab)/a1;
            ...
            % Terminal apoplast and symplast potentials on adaxial side 
            ...
            % Terminal apoplast and symplast potentials on abaxial side 
        ...
        kappa_mem_exp=floor(log10(kappa_mes_mem));
        kappa_mem_a=kappa_mes_mem/(10^kappa_mem_exp);
        ...
        title(sprintf('\\kappa_{mem} = %.2f \\times 10^{%d} mol/m/s/MPa',kappa_mem_a,kappa_mem_exp),'FontSize',20,'Interpreter','tex');
